...
# Make freetype from source in the tar subdirectory
# Syntax:
#   make-freetype.sh ${INPUT_TAR} ${OUTPUT_DIR}
...
then
	echo "Syntax error: make-freetype.sh {INPUT_TAR_GZ} {OUTPUT_FOLDER}"
	exit 1
fi
...
echo "Built freetype with be installed at ${PREFIX}"
...
# make symbolic link
rm -f ${BUILD}/freetype-current
ln -s ${PREFIX} ${BUILD}/freetype-current
exit 0
