...
# Syntax:
#   make-libjpeg.sh ${INPUT_TAR} ${OUTPUT_DIR}
...
TARZ=${1}
BUILD=${2}
VERSION=`basename ${TARZ} | sed 's/\.tar\.gz//' | sed 's/src.v/-/'`

# Check for the TAR file to make sure it exists
if [ "${TARZ}XX" == "XX" ] || [ ! -e ${TARZ} ]
then
	echo "Syntax error: make-libjpeg.sh {INPUT_TAR_GZ} {OUTPUT_FOLDER}"
	exit 1
fi
...
echo "Built libjpeg with be installed at ${PREFIX}"
...
# make symbolic link
rm -f ${BUILD}/libjpeg-current
ln -s ${PREFIX} ${BUILD}/libjpeg-current
exit 0
